...
@onready var grasshopper_node  = get_node("Dust Mite")
...
func _ensure_dialog_connection():
	...
	if not already_connected:
		print("Connecting bedmite to dialog system...")
		DialogSystem.connect("dialog_finished", Callable(self, "_on_dialog_finished"))
	else:
		print("Bedmite already connected to dialog system")

func _on_dialog_finished(finished_npc_id):
	print("Bedmite received dialog_finished signal for npc_id:", finished_npc_id)
...
func start_dialog():
	...
	if (not have_spoken() and not story_manager.found_controller):
		...
		story_manager.spoke_bedmite = true
		story_manager.fed_bedmite = true
