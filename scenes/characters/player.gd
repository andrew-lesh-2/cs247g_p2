...
const GROUND_POUND_COUNTDOWN = .2
const GROUND_POUND_COOLDOWN = .3
var ground_pound_cooldown = 0
var ground_pound_timer = 0
...
func _process(delta):
	...
	print(anim.position)
	if is_ground_pounding or ground_pound_cooldown > 0:
		anim.play("ground_pound")
		var progress = ground_pound_timer / GROUND_POUND_COUNTDOWN
		if progress < 1.0 / 3.0:
			anim.frame = 0
			...
			anim.frame = 3
		anim.position = Vector2(0, int(4 * max(1, ground_pound_timer / GROUND_POUND_COUNTDOWN)))
...
func _physics_process(delta):
	...
	if ground_pound_cooldown > 0:
		ground_pound_cooldown -= delta
		return

	super._physics_process(delta)

	if is_ground_pounding and ground_pound_timer < GROUND_POUND_COUNTDOWN:
		velocity.y = -50
		velocity.x = 0
	...
	if Input.is_action_just_pressed("ui_down") and not is_on_floor() and not is_ground_pounding:
		...
		velocity.y = 0  # Fast downward velocity
